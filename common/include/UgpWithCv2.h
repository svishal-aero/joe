...
#define FOR_ICV_G1G2_ONLY for (int icv =     ncv; icv <   ncv_gg; ++icv)
...
public:
  ...
  // Make sure we have gradients in ncv:ncv_gg-1

   double (*buf) = new double[ncv_ggff];
   FOR_I3 {
    FOR_ICV
        buf[icv] = gradPhi[icv][i];
          ...
          const int limiter, const double *refValue, const double epsilon)
  {
   ...
   // Make sure we have gradients in ncv:ncv_g-1
   double (*buf) = new double[ncv_ggff];
   FOR_I3 {
    FOR_J3 {
     FOR_ICV
        buf[icv] = gradPhi[icv][i][j];
  ...
  void limitCv2GradBJ(double (*grad_p)[3], const double *phi, const double *refValue, const double epsilonSDWLS)
  {
    for (int icv = 0; icv < ncv_g; icv++)
    {
      ...
      if ((phiMax-phiMin) > 1.0e-12)
      {
	    ...
	    //if(x_cv[icv][0]>31.4 && x_cv[icv][0]<32. && x_cv[icv][2]>2. && x_cv[icv][2]<2.8)
            //eps2 = max(1.0*fabs(refValue[icv]), SDWLSClippingScalar);
	    //else
            eps2 = max(epsilonSDWLS*fabs(refValue[icv]), SDWLSClippingScalar);
    ...
    for (int icv = 0; icv < ncv_g; icv++)
    {
      for (int i=0; i<3; i++)
      {
        ...
        if ((phiMax-phiMin) > 1.0e-8)
        {
	     ...
	     // if(x_cv[icv][0]>31.4 && x_cv[icv][0]<32. && x_cv[icv][2]>2. && x_cv[icv][2]<2.8)
             // eps2 = pow(1.0*refValue[icv], 2.0);
	     // else
              eps2 = pow(epsilonSDWLS*refValue[icv], 2.0);
        ...
        //if(grad_p[icv][0][0]+grad_p[icv][1][1]+grad_p[icv][2][2]<-0.1) alfa=0.0;
