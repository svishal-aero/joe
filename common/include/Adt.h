...
void Adt<T>::initAdt(int n,const T (*bbmin)[3],const T (*bbmax)[3]) {
  ...
  for (i = 1; i < n; i++) {
    for (j = 0; j < 3; j++) {
      leafdata[ileaf].bbmin[j] = std::min(leafdata[ileaf].bbmin[j],bbmin[i][j]);
      leafdata[ileaf].bbmax[j] = std::max(leafdata[ileaf].bbmax[j],bbmax[i][j]);
    }
  }
  ...
  if (n > 1) {
    ...
    while (istack > 0) {
      ...
      for (j = 0; j < 3; j++) {
        ...
        for (i = i_f+1; i <= i_l; i++) this_max = std::max(this_max,bbmin[index[i]][j]);
        ...
        for (i = i_f+1; i <= i_l; i++) this_min = std::min(this_min,bbmax[index[i]][j]);
      ...
      */
      ...
      if (i_mid > i_f) {
        ...
        for (i = i_f+1; i <= i_mid; i++) {
          this_index = index[i];
          for (j = 0; j < 3; j++) {
            leafdata[ileaf].bbmin[j] = std::min(leafdata[ileaf].bbmin[j],bbmin[this_index][j]);
            leafdata[ileaf].bbmax[j] = std::max(leafdata[ileaf].bbmax[j],bbmax[this_index][j]);
          }
        }
        if (istack == nstack) {
