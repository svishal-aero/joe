#include "../include/JoeWithModels.h"
#include "../../common/include/myMem.h"



void JoeWithModels::set_global_id(){
	
	printf("Setting GLOBAL ID\n");
	
	if(mpi_size != 2){
		
		fprintf(stderr, "NUMBER OF PROCESSORS SHOULD BE EXACTLY 2\n");
		throw(-1);
	...
	if(mpi_rank == 0){
		...
		MPI_Send(&ncv,1,MPI_INT,1,123,mpi_comm);	
		MPI_Recv(&rcv,1,MPI_INT,1,123,mpi_comm,&statuss);
	
	}
	
	else{
		
		MPI_Send(&ncv,1,MPI_INT,0,123,mpi_comm);	
		MPI_Recv(&rcv,1,MPI_INT,0,123,mpi_comm,&statuss);
	...
	for(int icv=0; icv<ncv; icv++){
		...
		global_id[icv] = rcv*mpi_rank + icv;
...
void JoeWithModels::write_global_id(){
	...
	if ( mpi_rank == 0 ) {
		...
		if ( (fp=fopen("global_id.dat","wt"))==NULL ) {
					
			fprintf(stderr, "Error: Cannot open file \"global_id.dat\" to write\n");
			throw(-1);
		
		}
			
		fprintf(fp, "# \"GlobalID\" \"X\" \"Y\" \"Z\"\n");

		if ( (fp_i=fopen("beta_init.dat","wt"))==NULL ) {

			fprintf(stderr, "Error: Cannot open file \"beta_init.dat\" to write\n");
			throw(-1);
		...
		if ( (fp=fopen("global_id.dat","a"))==NULL ) {
		...
		if ( (fp_i=fopen("beta_init.dat","a"))==NULL ) {
	...
	for(int icv=0; icv<ncv; icv++) {
		...
		fprintf(fp,   "%d\t%.12le\t%.12le\t%.12le\n",                 int(global_id[icv]),  x_cv[icv][0], x_cv[icv][1], x_cv[icv][2]);
		fprintf(fp_i, "%d\t%.12le\t%.12le\t%.12le\t%.12le\t%.12le\n", int(global_id[icv]),  x_cv[icv][0], x_cv[icv][1], x_cv[icv][2], 1.0, 0.0);
...
void JoeWithModels::WriteFlowFeatures(){
	if(mpi_size > 1){
		if (mpi_rank == 0)
			cout << "ERROR: calc_param is only supported on single processor!" << endl;
		throw(-1);
	}
	...
	WriteFlowFeaturesHook();
    writeData(0);

	//nparams = 4;
	FILE *fp;
	fp = fopen("flow_features.dat", "w");
	for(int icv=0; icv<ncv; icv++){
		fprintf(fp, "%d %.14f %.14f %.14f", int(global_id[icv]), x_cv[icv][0], x_cv[icv][1], x_cv[icv][2]);
		for(int i=0; i<nparams; i++){
			fprintf(fp, " %.14f", params[i][icv]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
...
void JoeWithModels::setGlobalId(){
	// read mesh or restart file
	initializeFromRestartFile(getStringParam("RESTART"));
	...
	if (!checkScalarFlag((char*)("global_id"))){
		this->set_global_id();
		this->write_global_id();
	}
	else{
		cout << "GLOBAL ID EXISTS!!!!\n" << endl;
		this->set_global_id();
		write_global_id();
		//		MPI_Finalize();
	}
...
void JoeWithModels::run(int rest)
{

	if(mpi_rank==0) printf("------------------------------------------------------------\n");
	if(mpi_rank==0) printf(" Restart file: %s\n", getStringParam("RESTART").c_str());
	if(mpi_rank==0) printf("------------------------------------------------------------\n");
...
void JoeWithModels::runForwardEuler()
{
  ...
  if (nScal > 0) getMem2D(&drhoScal, 0, nScal-1, 0, ncv-1, (char*)("drhoScal"));
...
void JoeWithModels::runBackwardEuler()
{
  ...
  */
  ...
  double **dScal   = NULL;  if (nScal > 0) getMem2D(&dScal,   0, nScal-1, 0, ncv_g-1, (char*)("dScal"));
  double **rhsScal = NULL;  if (nScal > 0) getMem2D(&rhsScal, 0, nScal-1, 0, ncv-1, (char*)("rhsScal"));
  ...
  while (done != 1)
  {
	...
	*/
	...
	double iScal_kine = getScalarTransportIndex("kine");
        double iScal_v2   = getScalarTransportIndex("v2");
        if(iScal_kine>=0 && iScal_v2>=0) {
        if(mpi_rank==0 && step%10000000000 ==0 )
        cout<<"Clipping v^2"<<endl;
        double *phi_kine = scalarTranspEqVector[iScal_kine].phi;
        double *phi_v2   = scalarTranspEqVector[iScal_v2].phi;
                for (int icv = 0; icv < ncv; icv++)
                        phi_v2[icv]=min(phi_v2[icv],2./3.*phi_kine[icv]);
                        updateCvDataG1G2(phi_v2, REPLACE_DATA);
        }
    ...
    for (int iScal = 0; iScal < nScal; iScal++)
      for (int icv = 0; icv < ncv; icv++)
        myResidual[5+iScal] += fabs(rhsScal[iScal][icv]/scalarTranspEqVector[iScal].relax);
...
void JoeWithModels::runBDF2()
{
  ...
  */
  ...
  double **dScal    = NULL;  if (nScal > 0) getMem2D(&dScal,    0, nScal-1, 0, ncv_g-1, (char*)("dScal"));
  double **rhsScal  = NULL;  if (nScal > 0) getMem2D(&rhsScal,  0, nScal-1, 0, ncv-1, (char*)("rhsScal"));
  double **qnScal   = NULL;  if (nScal > 0) getMem2D(&qnScal,   0, nScal-1, 0, ncv-1, (char*)("qnScal"));
  double **qnm1Scal = NULL;  if (nScal > 0) getMem2D(&qnm1Scal, 0, nScal-1, 0, ncv-1, (char*)("qnm1Scal"));
...
void JoeWithModels::calcRhs(double *rhs_rho, double (*rhs_rhou)[3], double *rhs_rhoE, double **rhs_rhoScal, double (*A)[5][5], double ***AScal, int flagImplicit)
{
  ...
  for (int iScal = 0; iScal < scalarTranspEqVector.size(); iScal++)
  {
	...
	/*
	FOR_ICV
        if(iScal==1 && x_cv[icv][0]>10.4398 && x_cv[icv][0]<10.4401 && x_cv[icv][1]>1.788275e-1  && x_cv[icv][1]<1.788276e-1)
        printf("%.16e %.16e %.16e\n", x_cv[icv][0],x_cv[icv][1],rhs_rhoScal[iScal][icv]);
	*/
    }
  }
}
    ...
    if (zone->getKind() == FA_ZONE_BOUNDARY)
    {
	  ...
	  for (int ifa = zone->ifa_f; ifa <= zone->ifa_l; ++ifa) {
	    ...
	    */
	    ...
	    if (rho[icv1] <= 0.0) {
	      cout << "got negative rho[icv1]: " << rho[icv1] << " at icv1 = "<<icv1<<" "<<x_cv[icv1][0]<<" "<<x_cv[icv1][1]<<endl;
	      throw(-1);
	    }
...
void JoeWithModels::calcViscousFluxNS(double *rhs_rho, double (*rhs_rhou)[3], double *rhs_rhoE, double (*A)[5][5], int flagImplicit)
{
  ...
  for (int ifa = nfa_b; ifa < nfa; ifa++)
  {
    ...
    addViscFlux_RIJ(Frhou, FrhoE, A0, A1,
              rho[icv0], vel[icv0], grad_u[icv0], enthalpy[icv0], grad_enthalpy[icv0], temp[icv0], RoM[icv0], gamma[icv0], kine0,
              rho[icv1], vel[icv1], grad_u[icv1], enthalpy[icv1], grad_enthalpy[icv1], temp[icv1], RoM[icv1], gamma[icv1], kine1,
    ...
    if (zone->getKind() == FA_ZONE_BOUNDARY)
    {
	  ...
	  if ((param->getString() == "SYMMETRY") || (param->getString() == "NOTHING"))
	    ...
	    //int ifa = zone->faVec[index];
	     ...
	     double test1 = 0; double test2 = 0; double test3 = 0;
             addViscFlux_RIJ(Frhou, FrhoE, A0, NULL, 
                      rho[icv0],    vel[icv0],    grad_u[icv0], enthalpy[icv0], grad_enthalpy[icv0], temp[icv0], RoM[icv0],    gamma[icv0],  kine0,
                      rho[icv1],    vel[icv1],    grad_u[icv0], enthalpy[icv1], grad_enthalpy[icv0], temp[icv1], RoM[icv1],    gamma[icv1], kine1,
                      nonLinear[ifa], rij_diag_fa[ifa], rij_offdiag_fa[ifa], mul_fa[ifa], 0.0, lamOcp_fa[ifa], kine_fa, vel[icv1],
	  ...
	  //int ifa = zone->faVec[index];
	     ...
	     double test1 = 0; double test2 = 0; double test3 = 0;
             addViscFlux_RIJ(Frhou, FrhoE, A0, NULL, 
                      rho[icv0],    vel[icv0],    grad_u[icv0], enthalpy[icv0], grad_enthalpy[icv0], temp[icv0], RoM[icv0],    gamma[icv0],  kine0,
                      rho[icv1],    vel[icv1],    grad_u[icv0], enthalpy[icv1], grad_enthalpy[icv0], temp[icv1], RoM[icv1],    gamma[icv1], kine1,
                      nonLinear[ifa], rij_diag_fa[ifa], rij_offdiag_fa[ifa], mul_fa[ifa], mut_fa[ifa], lamOcp_fa[ifa], kine_fa, vel[icv1],
                      area, nVec, smag, nVec, test1, test2, test3);  // <- use nVec here instead of sVec, to avoid inaccurate correction
	     }
...
void JoeWithModels::runBackwardEulerCoupled()
{
  ...
  double ***A;           getMem3D(&A,   0, nbocv_s-1, 0, 5+nScal-1, 0, 5+nScal-1, (char*)("JoeWithModels::runBackwardEulerCoupled -> A"),   true);
  double **rhs;          getMem2D(&rhs, 0, ncv-1,     0, 5+nScal-1,               (char*)("JoeWithModels::runBackwardEulerCoupled -> rhs"), true);
  double **dq;           getMem2D(&dq,  0, ncv_g-1,   0, 5+nScal-1,               (char*)("JoeWithModels::runBackwardEulerCoupled -> dq"),  true);
    ...
    for (int iScal = 0; iScal < nScal; iScal++)                          
     {
      ...
      for (int icv = 0; icv < ncv; ++icv)
      {
        if (iScal == getScalarTransportIndex("omega") || iScal == getScalarTransportIndex("nuSA") || iScal == getScalarTransportIndex("eps")) residField2[icv] = rhs[icv][5+iScal];
        if (iScal == getScalarTransportIndex("kine"))  residField3[icv] = rhs[icv][5+iScal];
        if (iScal == getScalarTransportIndex("ZMean") || iScal == getScalarTransportIndex("v2")) residField4[icv] = rhs[icv][5+iScal];
        if (iScal == getScalarTransportIndex("ZVar") || iScal == getScalarTransportIndex("f"))   residField5[icv] = rhs[icv][5+iScal];
...
void JoeWithModels::calcFluxCoupled(double **rhs, double ***A, int nScal, int flagImplicit)
{
  ...
  if (flagImplicit)
  {
    getMem2D(&Apl, 0, 5+nScal-1, 0, 5+nScal-1, (char*)("JoeWithModels::calcFluxCoupled -> Apl"), true);
    getMem2D(&Ami, 0, 5+nScal-1, 0, 5+nScal-1, (char*)("JoeWithModels::calcFluxCoupled -> Ami"), true);
    for (int i = 0; i < 5+nScal; i++)
      for (int j = 0; j < 5+nScal; j++)
      {
        Apl[i][j] = 0.0;
    ...
    if (mu_ref > 0.0)
    {
      getMem2D(&A0,  0, 5+nScal-1, 0, 5+nScal-1, (char*)("JoeWithModels::calcFluxCoupled -> A0 "), true);
      getMem2D(&A1,  0, 5+nScal-1, 0, 5+nScal-1, (char*)("JoeWithModels::calcFluxCoupled -> A1 "), true);
      for (int i = 0; i < 5+nScal; i++)
        for (int j = 0; j < 5+nScal; j++)
        {
          A0[i][j] = 0.0;
