...
//***************************************************************
{
    ...
    else if (tIntName == "SPARSEIO")               solveADSystem_IO();
    else
    {
       if (mpi_rank == 0)
       {
...
//******************************************************************
{
 ...
 //-************************* INITIALIZATION ****************************
  ...
  //=============== Sparse ===================
	...
	if(checkParam("WRITE_JACOBIAN")){
		cout<<" WRITING JACOBIAN"<<endl;
		cout<<" dims2 " << dims2 << endl;
		
		int *cidx = new int[nnz];
		int *ridx = new int[nnz];
		int *gid_ncv = new int[nnz];
		for(int i=0; i<ncv; i++){
			gid_ncv[i] = int(global_id[i]);
		}
		for(int i = 0; i < nnz; i++){
			ridx[i] = int(rind[i]);
			cidx[i] = int(cind[i]);
		...
		// cnpy::npz_save("jacobian_sparse_gidx.npz","data",&gid_ncv[0],{ncv},"w");
		// cnpy::npz_save("jacobian_sparse_ridx.npz","data",&ridx[0],{nnz},"w");
		// cnpy::npz_save("jacobian_sparse_cidx.npz","data",&cidx[0],{nnz},"w");
		// cnpy::npz_save("jacobian_sparse_drdq.npz","data",&values[0],{nnz},"w");
		// cnpy::npz_save("jacobian_sparse_djdq.npz","data",&functional_der[0],{dims2},"w");
		...
		// for(int i = 0; i < nnz; i++)
		// 	fprintf(fpt,"%d %d %18.18e\n",int(global_id[rind[i]]),int(global_id[cind[i]]),values[i]);
		// fclose(fpt);
		
		// fpt = fopen("Vec.dat","w");
		// for(int i = 0; i < dims2; i++){
		// 	fprintf(fpt,"%d %18.18e\n", int(global_id[i]), functional_der[i]);
		// }
		// fclose(fpt);

		// UNCOMMENT THIS TO PREVENT MEMORY LEAK
		delete[] ridx;
		delete[] cidx;
		delete[] gid_ncv;
	}
    ...
    for(int i = 0; i < ncv; i++){
		fprintf(fpt,"%d %d %18.18e %18.18e\n", i, int(global_id[i]),x_cv[i][0],x_cv[i][1]);
    }
    fclose(fpt);
	*/
...
//***************************************
       ...
       solveLinSysFull(dadj_vars, values, rhs_adj, cind1, rind1, zeroAbs, zeroRel, maxIter, nAdjvars,1);
	...
	if(mpi_rank==0) cout<<"STEPS BY 20"<<endl;

    if ((nsteps != -1) && (iter >= nsteps/20))
      done = 1;
  }
...
{
    int index = 0;
    while ( index < size && x[index] != value ) ++index;
    return ( index == size ? -1 : index );
}

//*************** Explicit Euler solution of AD system  ************
void JoeWithModels_AD::solveADSystem_IO()
//******************************************************************
{
//assert(mpi_size == 1);
//    int nScal = scalarTranspEqVector.size();
...
//	for (int iScal = 0; iScal < nScal; iScal++){
//		scalarTranspEqVector[iScal].coupling = "COUPLED";
//		//cout<<scalarTranspEqVector[iScal].name<<endl;
//	}
//	calcResidualDerivative_Coupled(A,0);
//
//	initialize_turb_adjoint();
//	initialize_comb_adjoint();
//
//	cout<<"Solve AD IO"<<endl;
//
//	cnpy::npz_t data_vec = cnpy::npz_load("adjoint_vector.npz");
//	cnpy::NpyArray np_vec = data_vec["data"];
//	double *adjoint_vec = np_vec.data<double>();
//
//	cnpy::npz_t data_gidx = cnpy::npz_load("jacobian_sparse_gidx.npz");
//	cnpy::NpyArray np_gidx = data_gidx["data"];
//	int *gid_ncv = np_gidx.data<int>();
//	int gid_ncv_size = np_gidx.shape[0];
//	cout<<"calling zero turb adjoint"<<endl;
//	zero_turb_adjoint();
//	cout<<"done calling zero turb adjoint"<<endl;
//
//     for (int icv = 0; icv < ncv; icv++){
//		 int _gid = int(global_id[icv]);
//		 int idx = find_index(gid_ncv, gid_ncv_size, _gid);
//		 int ii = nAdjvars*idx;
//		 //if(icv<1000)
//		 //	 cout<<" ii "<<ii<<" gid "<<_gid<< " idx "<< idx<<endl;
//		 psi_rho[icv]    =adjoint_vec[ii];
//		 psi_rhou[icv][0]=adjoint_vec[ii+1];
//		 psi_rhou[icv][1]=adjoint_vec[ii+2];
//		 psi_rhou[icv][2]=adjoint_vec[ii+3];
//		 psi_rhoE[icv]   =adjoint_vec[ii+4];
//		 for(int iScal=0; iScal<nScal; iScal++){
//			 scalarTranspEqVector_psi[iScal].phi[icv]= adjoint_vec[ii+5+iScal];
//		 }
//	 }
