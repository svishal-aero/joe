...
public:   // member variables
   ...
   double *psi_gammai;      ///< Adjoint intermittency
...
public:   // member functions
   ...
   virtual void initialHookScalarRansTurbModel_AD()
  {
      ...
      virtual void write_turb_adjoints_AD()
  {
  }
 ...
 // \brief Compute for a given temperature the properties of the mixture at a face.
  ...
  void calcEulerFluxMatrices_HLLC_AD(double (*A_L)[5], double (*A_R)[5], double (*A_L_Scal)[6], double (*A_R_Scal)[6],
   ...
   virtual void addViscFluxJacobians(double (*A0)[5], double (*A1)[5],
    ...
    virtual void sourceHookCoupled_AD(REALA **rhs,          double ***A,  int flagImplicit) {/* empty */
		if(checkParam("AXISYMMETRIC")){
			...
			axisymmetric_source_coupled_AD(rhs, A, flagImplicit);
		}
	}
    
	virtual void axisymmetric_source_coupled_AD(adouble **rhs, double ***A, int flagImplicit)  { 
		...
		for(int icv = 0; icv < ncv; icv++){
			...
			rhs[icv][3] -= 0.0;
			rhs[icv][4] -= (rhoE_AD[icv] + _p)*_v*_fac;
    ...
    virtual void copy_turb_adjoint() {cout<<"copy turb adjoint ugp"<<endl;/* empty */}
    virtual void zero_turb_adjoint() {cout<<"zero turb adjoint ugp"<<endl;/* empty */}
    ...
    void calcEulerFluxMatricesCoupled_HLLC_AD(double **A_L, double **A_R,
